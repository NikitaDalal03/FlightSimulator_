        ...
        public void Initialize()
        {
            ...
            if (allOk)
            {
                ...
                if (controlState == ControlType.Controllable && m_foilType == AerofoilType.Wing)
                {
                    if (flapState == ControlState.Active)
                    {
                        if (_flapClamp) { Handler.SetupSoundSource(this.transform, _flapClamp, "_flap_clamp", 80f, false, false, out _flapClampSource); _flapClampSource.volume = 0.1f; }
                        if (_flapLoop) { Handler.SetupSoundSource(this.transform, _flapLoop, "_flap_loop", 80f, true, false, out _flapLoopSource); _flapLoopSource.volume = 0.1f; }
                    }
                    if (slatState == ControlState.Active)
                    {
                        if (_slatClamp) { Handler.SetupSoundSource(this.transform, _slatClamp, "_slat_clamp", 80f, false, false, out _slatClampSource); _slatClampSource.volume = 0.1f; }
                        if (_slatLoop) { Handler.SetupSoundSource(this.transform, _slatLoop, "_slat_loop", 80f, true, false, out _slatLoopSource); _slatLoopSource.volume = 0.1f;}
                    }
                }
            }
        }
        ...
        void AnalyseSound(int mode, float target)
        {
            if (m_foilType == AerofoilType.Wing)
            {
                if (_aircraft != null && _aircraft.m_view != null)
                {
                    ...
                    {
                        if (_flapClampSource != null && _flapLoopSource != null) { _flapLoopSource.volume = 0.1f; _flapClampSource.volume = 0.1f; }
                        if (_slatClampSource != null && _slatLoopSource != null) { _slatLoopSource.volume = 0.1f; _slatClampSource.volume = 0.1f; }
                    }
