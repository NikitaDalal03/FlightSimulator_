    ...
    public class SilantroActuator : MonoBehaviour
    {
        ...
        public void Initialize()
        {
            if (actuatorAnimator != null)
            {
                ...
                if (EngageLoopClip) { Handler.SetupSoundSource(this.transform, EngageLoopClip, "Loop Point", 150f, true, false, out EngageLoopPoint); /*EngageLoopPoint.volume = 0.1f;*/ }
                if (EngageEndClip) { Handler.SetupSoundSource(this.transform, EngageEndClip, "Clip Point", 150f, false, false, out EngageEndPoint); /*EngageEndPoint.volume = 0.1f; */}
                if (EngageClip) { Handler.SetupSoundSource(this.transform, EngageClip, "Clip Point", 150f, false, false, out actuationSoundPoint); /*actuationSoundPoint.volume = 0.1f;*/ }
                initialized = true;
            }
