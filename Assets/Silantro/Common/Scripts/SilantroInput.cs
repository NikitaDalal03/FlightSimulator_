...
namespace Oyedoyin.Common
{
    ...
    {
        public static SilantroInput instance;
        ...
        public float keyboardRollInput;
        ...
        public void Compute()
        {
            ...
            if (_controller.m_inputType == Controller.InputType.Default && _controller.m_inputLogic == Controller.InputLogic.Legacy && inputConfigured)
            {
                ...
                if (Application.isFocused)
                {
                    ...
                    //Debug.Log("RawPitch: " + _rawPitchInput);
            ...
            {
                // Check if any of the connected touch controllers are pressed
                ...
                //Up down
                if (Input.GetKey(KeyCode.S)) { Debug.Log("Inside Up S"); _pitchInput += 0.01f; _pitchInput = Mathf.Clamp(_pitchInput, -1f, 1f); }
                else if (Input.GetKey(KeyCode.W)) { Debug.Log("Inside Down W");  _pitchInput -= 0.01f; _pitchInput = Mathf.Clamp(_pitchInput, -1f, 1f); }
                else {  _pitchInput = 0f;  }
              
                //left right
                if (Input.GetKey(KeyCode.D)) { Debug.Log("Inside left D");  _rollInput += 0.01f;  _rollInput = Mathf.Clamp(_rollInput, -1f, 1f); }
                else if (Input.GetKey(KeyCode.A)) { Debug.Log("Inside right A");  _rollInput -= 0.01f;  _rollInput = Mathf.Clamp(_rollInput, -1f, 1f);  }
                else {  _rollInput = 0f; }
                ...
                {
                    // Turn left
                    Debug.Log("Turning Left with Q");
                    keyboardRollInput -= 0.01f;
                    Debug.Log("_rawYawInput: " + keyboardRollInput);
                    _rawYawInput = Mathf.Clamp(keyboardRollInput, -1f, 1f);
                }
                else if (Input.GetKey(KeyCode.E))
                {
                    // Turn right
                    Debug.Log("Turning Right with E");
                    keyboardRollInput += 0.01f;
                    Debug.Log("_rawYawInput: " + keyboardRollInput);
                    _rawYawInput = Mathf.Clamp(keyboardRollInput, -1f, 1f);
                }
                ...
                {
                    // Gradually reset the input towards zero
                    if (keyboardRollInput > 0)
                    {
                        keyboardRollInput -= 0.1f;
                        if (keyboardRollInput < 0) keyboardRollInput = 0;
                    }
                    else if (keyboardRollInput < 0)
                    {
                        keyboardRollInput += 0.1f;
                        if (keyboardRollInput > 0) keyboardRollInput = 0;
                    }
            ...
            if (_controller.m_inputType == Controller.InputType.VR)
            {
                ...
                if (m_throttleLever != null) { _throttleInput = m_throttleLever.leverOutput; } else { _throttleInput = 0; }
